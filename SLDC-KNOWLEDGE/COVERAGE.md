...
| Performance profiling / tracing / diagnostics | verified representative | `performance-profiling-tracing.md` | Perfetto tracing/profiling + automated SQL analysis, Go pprof/PGO, JDK 26 JFR, Windows WPR/WPA/ETW, Linux `perf`/`perf_events`, Linux eBPF continuous profiling with Grafana Alloy/Pyroscope, and bounded AI-assisted ETW/flame-graph interpretation |
...
- additional fuzzing systems, language-native fuzzers, macOS Instruments, GPU/vendor profilers, independent cross-tool continuous-profiler overhead/accuracy comparisons, and statistically robust noisy-performance CI methods;
