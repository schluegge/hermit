...
| Immutable / transactional Linux OS updates | verified representative | `immutable-transactional-linux-updates.md` | rpm-ostree staged deployments/rollback and `/var` boundary; bootc atomic OCI-image update/rollback with trust/migration caveats; NixOS retained generations/rollback/GC boundary |
...
- app-store/fleet/desktop/OS promotion automation that directly consumes crash/SLO/update-health evidence for promotion or rollback decisions;
- further evidence-backed AI security/fuzzing/performance/release/deployment/incident/database/ML/data-lineage/OS-maintenance automation, with generated outputs kept behind deterministic verification and explicit authority boundaries.